#!/usr/bin/env python3
"""Curated persistent memory for Codex."""
...
def search_messages(
    ...
    results: list[dict[str, Any]] = []
    if not substring:
        results = fulltext_search_messages(con, query, limit)
    if trigram and should_try_trigram(query) and (substring or len(results) < limit):
        results = merge_results(results, trigram_search_messages(con, query, limit), limit)
    if len(results) < limit:
        results = merge_results(results, like_search_messages(con, query, limit), limit)
    return results
...
def fulltext_search_messages(
    ...
    try:
        rows = con.execute(
            """
            SELECT msg.id, msg.session_id, msg.role, msg.content, msg.created_at,
...
def like_search_messages(
    ...
    rows = con.execute(
        """
        SELECT id, session_id, role, content, created_at, 0 AS rank, 'like' AS match_type
        FROM messages
        WHERE content LIKE ?
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Codex curated memory store")
