...
1. `CLAUDE_MEMORY_DB`
2. `$CLAUDE_HOME/memories/memory_state.db`
3. `~/.claude/memories/memory_state.db`

The database is global to Claude Code and independent from project databases. Do not store application secrets, credentials, API keys, private tokens, raw `.env` values, or unnecessary personal data.
...
```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py init
```

Start a session:

```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py start-session --title "Implement API change" --project-path "/path/to/project"
```

Log a message:

```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py add-message --session-id "ses_..." --role user --content "Change the API envelope." --relevance 1
```
...
```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py search "API contract" --scope project --project-path "/path/to/project" --include-messages
```
...
```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py search "velope" --scope project --project-path "/path/to/project" --include-messages --substring
```

Remember:

```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py remember --scope project --project-path "/path/to/project" --kind decision --key "api-envelope" --content "API routes return JSON envelopes shaped as { data, error }." --confidence 1.0 --session-id "ses_..." --message-id "msg_..."
```
...
```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py list --scope project --project-path "/path/to/project"
```

Forget:

```bash
python ~/.claude/skills/curated-memory-plus/scripts/memory.py forget --id "<memory-id>"
```
