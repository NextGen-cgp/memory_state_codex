...
- Claude Code has access to a global curated memory store managed by the `curated-memory` skill.
...
- The default skill script path is:
  - `$CLAUDE_HOME/skills/curated-memory/scripts/memory.py`, if `CLAUDE_HOME` is set;
  - `~/.claude/skills/curated-memory/scripts/memory.py`.
